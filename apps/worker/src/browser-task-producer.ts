...
type Target = { readonly deviceId: string; readonly candidateId: string } & (
  { readonly kind: "supplier_search" | "amazon_package" | "amazon_search" | "product_database" | "keyword_scout" | "historical_data" | "category_trends" } | { readonly kind: "supplier_detail"; readonly sourceCaptureId: string }
);
async function queueBrowserRead(pool: Pool, target: Target, signing: Signing) {
  ...
  try {
    ...
    const source = device ? (target.kind==='amazon_search'||target.kind==='product_database'||target.kind==='keyword_scout'||target.kind==='historical_data'||target.kind==='category_trends')?await lockMarketContext(db,target.candidateId):target.kind === 'amazon_package' ? await lockPackageContext(db,target.candidateId) : await lockSourcingContext(db, target.candidateId) : null;
    ...
    const request: BrowserReadTask["request"] = target.kind==='category_trends' && 'market_query' in source
...
export function queueHistoricalData(pool:Pool,target:{readonly deviceId:string;readonly candidateId:string},signing:Signing){
 return queueBrowserRead(pool,{...target,kind:'historical_data'},signing);
}
export function queueCategoryTrends(pool:Pool,target:{readonly deviceId:string;readonly candidateId:string},signing:Signing){
 return queueBrowserRead(pool,{...target,kind:'category_trends'},signing);
}
