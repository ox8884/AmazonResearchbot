...
function status(row: DeviceRow): BridgeDevice {
  const capabilities = [...new Set(row.reported_tasks)].filter((kind): kind is BridgeDevice["capabilities"][number] => kind === "supplier_search" || kind === "supplier_detail" || kind === "amazon_package" || kind === "saved_search_export" || kind === "amazon_search" || kind === "product_database" || kind === "keyword_scout" || kind === "historical_data" || kind === "category_trends");
