...
브라우저 변경 승인(2026-09-08): Jay가 BrowserOS 대신 오늘 설치한 ASIDE 사용을 지시했다. 이후 브라우저 어댑터는 ASIDE의 실제 지원 인터페이스를 기준으로 구현하며, 아래 BrowserOS/neo 명칭은 변경 전 설계 기록이다. 기기 인증·서명된 제한 작업·쿠키/프로필 비전송·외부 연락 승인 경계는 유지한다.
...
- 현재 작업: Jay가 프론트 개선을 확인하고 견적 입력·저장·손익 계산에 필요한 API·도메인·DB의 로컬 구현을 승인했다(2026-09-06). M3 중 수동 견적 입력·저장·평가 경로는 실제 API/DB/브라우저 실측과 독립 코드·시각 검토를 통과했다. 상세 증거는 docs/verification-quotes.md에 있다. 전체 M3/무인 소싱/발주 승인은 아직 완료가 아니다. 유료 호출·외부 전송·프로덕션 배포·데이터 삭제는 하지 않는다.
