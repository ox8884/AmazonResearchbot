...
try{
 ...
 async function candidate(label,representative=asin){
  ...
  await test.pool.query("UPDATE browser_tasks SET state='completed',result_id=$2,delivered_at=now(),first_delivered_at=COALESCE(first_delivered_at,now()) WHERE id=$1",[ciTask.id,ciResult.id]);
  return c.id;
 }
