...
for(const name of ['.env','.ENV.junglescout','ops/runtime.env','.omo/evidence/a.json','apps/web/qa/a.png','data/backup.fops','worker-authority.json','docs/verification-quotes-state.json'])assert.ok(privateSourcePath(name),name);
for(const name of ['.env.example','ops/runtime.env.example','ops/worker-authority.example.json','packages/db/sql/0030_summary_deliveries.sql'])assert.equal(privateSourcePath(name),null,name);
...
for(const token of ['ghp_'+'a'.repeat(36),'sk-proj-'+'b'.repeat(40)])assert.equal(inspectSource({path:'src/token.ts',mode:'100644',content:Buffer.from(token)}).length,1);
