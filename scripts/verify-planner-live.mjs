...
let child,output='';
async function until(probe,label,timeout=25000){const end=Date.now()+timeout;while(Date.now()<end){if(await probe())return;await delay(100);}throw new Error('Timed out: '+label);}
...
try{
 ...
 await until(async()=>Number((await test.pool.query('SELECT count(*)::int AS n FROM daily_summaries')).rows[0].n)===1,'summary after research jobs complete',70000);
