...
if (scenario !== "csv20" && scenario !== "all") {
  console.log(`scenario ${scenario}: not implemented in M1 (skipped)`);
...
const againList = againBody.candidates ?? (await api("/api/candidates")).body.candidates;
...
const afterBlock = (await api("/api/candidates")).body.candidates;
...
const wires = await pool.query(`SELECT count(*)::int AS n FROM api_attempts WHERE status = 'dispatching' OR status = 'succeeded'`);
