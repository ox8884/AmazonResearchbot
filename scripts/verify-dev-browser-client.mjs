...
import {acquireBridgeProcessLock} from '../apps/browser-bridge/process-lock.mjs';
const root=fileURLToPath(new URL('../',import.meta.url));
const directory=await mkdtemp(path.join(tmpdir(),'forge-dev-bridge-'));
const lockDirectory=await mkdtemp(path.join(tmpdir(),'forge-dev-lock-'));
const lockDeviceId=randomUUID();
await assert.rejects(acquireBridgeProcessLock(lockDirectory,'../../evil'),/INVALID_DEVICE_ID/);
const rebootStaleLock=path.join(lockDirectory,`.device-${lockDeviceId}.lock`);
await writeFile(rebootStaleLock,JSON.stringify({pid:process.pid}),{flag:'wx'});
await utimes(rebootStaleLock,new Date('2000-01-01T00:00:00Z'),new Date('2000-01-01T00:00:00Z'));
const releaseRebootStaleLock=await acquireBridgeProcessLock(lockDirectory,lockDeviceId);
await releaseRebootStaleLock();
const incompleteLock=path.join(lockDirectory,`.device-${lockDeviceId}.lock`);
await writeFile(incompleteLock,'',{flag:'wx'});
const publishOwner=setTimeout(()=>void writeFile(incompleteLock,JSON.stringify({pid:process.pid})),50);
await assert.rejects(acquireBridgeProcessLock(lockDirectory,lockDeviceId),/BRIDGE_CLIENT_ALREADY_RUNNING/);
clearTimeout(publishOwner);
await rm(incompleteLock,{force:true});
const releaseLock=await acquireBridgeProcessLock(lockDirectory,lockDeviceId);
await assert.rejects(acquireBridgeProcessLock(lockDirectory,lockDeviceId),/BRIDGE_CLIENT_ALREADY_RUNNING/);
await releaseLock();
const releaseRestartedLock=await acquireBridgeProcessLock(lockDirectory,lockDeviceId);
await releaseRestartedLock();
...
try{
 ...
 assert.ok(stderr.includes('DEVICE_CREDENTIAL_MISSING'),'NODE_OPTIONS must not hijack the child before its actual entrypoint');
 ...
 try{
  ...
  assert.equal(supervisor.coreCount,3);await supervisor.coreAlive();
 ...
 const degradedSupervisor=await openDevSupervisorFixture({root,source,exitedBackup:true});
 try{
  assert.equal(degradedSupervisor.coreCount,3);
  await degradedSupervisor.coreAlive();
 }finally{await degradedSupervisor.close();}
 ...
 console.log(JSON.stringify({scenario:'dev-browser-client',result:'PASS',unconfiguredSkipped:true,actualChildStarted:true,nodeOptionsNotForwarded:true,optionalFailureDoesNotStopCore:true,backupFailureLeavesCoreAlive:true,duplicateDeviceClientRejected:true,actualSupervisorTail:true,alreadyExitedCoreRecognized:true,coreStandInsClosedBySignalHandler:true,originMismatchRejected:true,productionRejected:true,externalCalls:0}));
}finally{
